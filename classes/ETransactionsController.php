...
class ETransactionsController extends ETransactionsAbstract {
    ...
    public function __construct() {
        parent::__construct(new ETransactionsEpayment());
    }
    ...
    public function cancelAction() {
        try {
            ...
            if ($params !== false) {
                ...
                $message = sprintf('Cart %d: Payment was canceled by user onE-Transactions payment page.', $cart->id);
                $this->logDebug($message);
                //$message = $this->l('Payment was canceled by user on E-Transactions payment page.');
    ...
    public function failureAction() {
        try {
            ...
            if ($params !== false) {
                $cart = $this->getHelper()->untokenizeCart($params['reference']);
                $message = sprintf('Cart %d: Customer is back fromE-Transactions payment page. Payment refused by E-Transactions (%d).', $cart->id, $params['error']);
                $this->logDebug($message);
                //$message = $this->l('Customer is back fromE-Transactions payment page.');
                //$message .= ' '.$this->l('Payment refused by ETransactions');
                // TODO no way to associate this message to an order.               
                //$message = $this->l('Payment refused by ETransactions');
                //$this->getHelper()->addCartErrorMessage($message);
...
*/
            ...
            // Payment refused
                ...
                //$message = $this->l('Payment was refused by E-Transactions (%s).');
                //$error = $this->getHelper()->toErrorMessage($params['error']);
    ...
    public function redirectAction() {
        ...
        <html>
            <body>
                <form action="<?php echo Tools::htmlentitiesUTF8($url); ?>" method="post" name="ETransactionsSystem" enctype="application/x-www-form-urlencoded">
                    <p><center>
                        <?php
                        if ($debug) {
                            echo $this->l('This is a debug view. Click continue to be redirected toE-Transactions payment page.');
                        } else {
                            echo $this->l('You will be redirected to theE-Transactions payment page. If not, please use the button bellow.');
                        }
                        ?>
                    </center></p>
        ...
        <?php
        if (!$debug) {
            echo '<script>document.forms["ETransactionsSystem"].submit();</script>';
        }
        ?>
        </body>
        </html>
    ...
    public function successAction() {
        try {
            // The loop is used to let E-Transactions call the IPN URL so that the order
            ...
            if ($params !== false) {
                ...
                $this->logDebug($message);
                // $message = $this->l('Customer is back from E-Transactions payment page.');
                // TODO no way to associate this message to an order...
                // RetroCompat 1.4
                if (version_compare(_PS_VERSION_, '1.5', '<')) {
