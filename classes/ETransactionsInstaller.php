...
class ETransactionsInstaller {
	...
	public function installOrderState($module) {
		//E-TRANSACTIONS ORDER TABLE
		if (!Db::getInstance()->Execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ETRANS_order` (
		...
		if (!Db::getInstance()->Execute('CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ETRANS_recurring` (
			`id_ETRANS_recurring` INT( 10 ) NOT NULL AUTO_INCREMENT ,
			`id_order` INT( 10 ) NOT NULL ,
			`status` VARCHAR( 255 ) NOT NULL ,
			`number_term` int(1) NOT NULL ,
			`amount_paid` INT( 15 ) NOT NULL ,
			PRIMARY KEY (  `id_ETRANS_recurring` ))
			ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8'))
			return false;
		...
		if (!$this->_isValidState(Configuration::get('ETRANS_ID_ORDER_STATE_NX')))
		{
			...
			foreach (Language::getLanguages() AS $language)
			{
				if (strtolower($language['iso_code']) == 'fr')
					$orderState->name[$language['id_lang']] = 'Payé partiellement via ETransactions';
				else
					$orderState->name[$language['id_lang']] = 'Partially paid through ETransactions';
			}
	...
	public function installETransactionsCard($module) {
		...
		$sql = 'CREATE TABLE IF NOT EXISTS `%sETRANS_card` (
	...
	public function uninstall($module) {
		...
		// - ETRANS_recurring
		// - ETRANS_order
		$tables = array('ETRANS_card', 'ETransactions');
